...
function renderHeroMedia(settings) {
  ...
  const videoWrapper = document.getElementById('hero-yt-player-wrapper');

  const videoId = (window.Utils && window.Utils.extractYouTubeId(youtubeUrl)) || 'qEnvCBBya-s';
...
function renderVideoSection(settings) {
  ...
  const sectionVideoUrl = settings?.video_section_youtube_url || settings?.hero_youtube_url || 'https://www.youtube.com/watch?v=qEnvCBBya-s&t=41s';
  const sectionVideoId = (window.Utils && window.Utils.extractYouTubeId(sectionVideoUrl)) || 'qEnvCBBya-s';
  const videoWrapper = document.getElementById('section-yt-player-wrapper');
...
function renderContactAndHours(settings, hours) {
  ...
  if (scheduleContainer && hours && hours.length > 0) {
    ...
    scheduleContainer.innerHTML = hours.map((day) => {
      ...
      return `
        <div class="flex items-center justify-between p-3 sm:p-3.5 rounded-xl transition-all schedule-row ${isToday ? 'bg-[#e0f7fa] border border-[#80deea] font-bold shadow-xs' : 'border border-transparent hover:bg-slate-50/80'}" data-day="${window.Utils.escapeHtml(day.day_name)}">
          <div class="flex items-center gap-3">
            <span class="w-2.5 h-2.5 ${isOpen ? 'bg-emerald-500' : 'bg-rose-500'} rounded-full flex-shrink-0"></span>
...
function renderSocialLinks(socials) {
  ...
  container.innerHTML = validSocials.map(s => {
    ...
    if (platform.includes('youtube') || url.includes('youtube') || url.includes('youtu.be')) {
    ...
    } else if (platform.includes('facebook') || platform.includes('fb') || url.includes('facebook.com') || url.includes('fb.com')) {
    ...
    } else if (platform.includes('instagram') || platform.includes('ig') || url.includes('instagram.com')) {
      ...
      hoverColor = 'hover:bg-gradient-to-tr hover:from-amber-500 hover:via-rose-500 hover:to-purple-600';
    } else if (platform.includes('tiktok') || url.includes('tiktok.com')) {
      // Official TikTok SVG
      ...
      // Default to YouTube red
      iconSvg = '<svg class="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>';
      hoverColor = 'hover:bg-[#FF0000]';
    }

    return `
      <a href="${window.Utils.escapeHtml(s.url)}" target="_blank" rel="noopener noreferrer" class="w-10 h-10 rounded-xl bg-slate-800 ${hoverColor} text-slate-300 hover:text-white flex items-center justify-center transition-all shadow-sm" aria-label="${window.Utils.escapeHtml(s.label || s.platform)}">
        ${iconSvg}
      </a>
    `;
  }).join('');
